...
public class AiLogic {
    ...
    public static int calculateScore(Board board, FigureColor whoMoves) {
        int score = 0;
        if (whoMoves == WHITE) {
...
//        System.out.println(score);
    ...
    public Move getBestMove(Board board, FigureColor whoMoves, int stepsCount) {
        ...
        boardCopy.move(makeMove.getCol1(), makeMove.getRow1(), makeMove.getCol2(), makeMove.getRow2());
        possibleMoves.clear();
        ...
        stepsCount--;
        if (stepsCount > 0) {
            return getBestMove(boardCopy, getOppositeColor(whoMoves), stepsCount);
        } else {
            return makeMove;
        }
    ...
    private boolean possibleHitsToLeftOrRight(
        ...
        if (notOutside) {
            if (board.getFigure(targetCol, row + 2 * direction).getColor() == FigureColor.NONE &&
                ...
                possibleMoves.add(new Move(col, row, hittedCol, row + 1 * direction));
            return true;
        }
        return false;
    }
